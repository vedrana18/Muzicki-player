...
/**
 * Klasa pokazuje dokle je doslo vrijeme i prati mjesto za slajder
 */
public class PlayingTimer extends Thread {
    ...
    //kontrola dinamicnih stvari (sto je inace kad se koristi Timer ono Action Listener
    @Override
    public void run() {
        isRunning = true;
        ...
        while (isRunning) {
            try {
                Thread.sleep(100); //Sto je u Timer onaj delay
                if (!isPause) {
    ...
    /**
     * Vraca brojanje na pocetak, to jest na 00:00:00
     */
    void reset() {
        isReset = true;
        isRunning = false;
    ...
    /**
     * Generise String za brojac vremena u formatu "HH:mm:ss"
     */
